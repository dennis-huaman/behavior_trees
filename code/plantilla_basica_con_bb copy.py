...
class GenericAction1(py_trees.behaviour.Behaviour):
    def __init__(self, name: str):
        ...
        # Definicion de variables que podrán ser accedidas o leidas por este nodo, y en qué modo (Lectura, Escritura)
        self.blackboard.register_key(
            key="variable1", access=py_trees.common.Access.WRITE
        )
        self.blackboard.register_key(
            key="variable2", access=py_trees.common.Access.READ
        )
    ...
    def update(self) -> py_trees.common.Status:
        ...
        try:
            self.blackboard.variable1 = random.randint(0, 100)
            new_status = py_trees.common.Status.SUCCESS
        except:
            new_status = py_trees.common.Status.FAILURE
        self.feedback_message = f"Valor de la variable1: {self.blackboard.variable1}"
        self.logger.debug(
...
class GenericAction2(py_trees.behaviour.Behaviour):
    def __init__(self, name: str):
        ...
        # Definicion de variables que podrán ser accedidas o leidas por este nodo, y en qué modo (Lectura, Escritura)
        self.blackboard.register_key(
            key="variable1", access=py_trees.common.Access.READ
        )
        self.blackboard.register_key(
            key="variable2", access=py_trees.common.Access.WRITE
        )
...
def create_root() -> py_trees.behaviour.Behaviour:
    ...
    # Asignacion de un nodo hijo a un nodo selector
    selector.add_child(nodo1)

    # Definicion de un nodo sequence
    sequence = py_trees.composites.Sequence(name="Sequence", memory=True)

    # Asignacion de nodos hijos a un nodo sequence
    sequence.add_children([nodo2, nodo3])
...
def main() -> None:
    ...
    # Inicializacion del blackboard con las variables a compartir
    blackboard = py_trees.blackboard.Client(name="My Blackboard")
    blackboard.register_key(key="variable1", access=py_trees.common.Access.WRITE)
    blackboard.register_key(key="variable2", access=py_trees.common.Access.WRITE)
    blackboard.variable1 = 0
    blackboard.variable2 = 0
    print(blackboard)
